...
# 
# TO DO
#  Auth on cognito user ID check will be needed here
  access_token = extract_access_token(request.headers)
  try:
    claims = cognito_jwt_token.verify(access_token)
    # authenicatied request
...
def data_create_message():
  ...
  app.logger.debug(request.json.get('handle', None))
  ...
  try:
    ...
    if message_group_uuid == None:
      ...
      app.logger.debug("UPDATING MSG GROUP : ", message_group_uuid)
      model = CreateMessage.run(
        mode="update",
        message=message,
        message_group_uuid=message_group_uuid,
    ...
    if model['errors'] is not None:
      app.logger.debug("errors in create_message, app.py line 204 : ")
...
@cross_origin()
def data_activities():
  # My new messages were hijacked by andrew's user handle!
